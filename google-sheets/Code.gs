...
function styleCore_(ss){
  ...
  [TAB_COMP,TAB_TORNEOS,TAB_CODIGOS,TAB_INS,TAB_PARTIDOS,TAB_WIN,TAB_MEDIA,TAB_CONFIG].forEach(name=>{
...
function registrations_(id){return rows_(ss_().getSheetByName(TAB_INS)).filter(r=>String(r.torneo_id)===String(id)&&upper_(r.estado||'CONFIRMADA')!=='CANCELADA')}
...
function state_(tournamentId,game){
  ...
  if(upper_(t.estado)!=='VIGENTE')return{ok:true,locked:true,registrations:[],matches:[],message:'Solo la edición VIGENTE expone participantes y fixture.'};
  const ss=ss_();
  const registrations=rows_(ss.getSheetByName(TAB_INS)).filter(r=>String(r.torneo_id)===tournamentId&&(!game||String(r.juego)===game)&&upper_(r.estado||'CONFIRMADA')!=='CANCELADA').map(r=>({createdAt:r.fecha_registro,tournamentId:String(r.torneo_id),game:String(r.juego),name:String(r.nombre),nick:String(r.nick),contact:String(r.contacto||''),location:String(r.ubicacion||''),tiktok:String(r.tiktok||''),photoUrl:String(r.foto_url||'')}));
  ...
  return{ok:true,locked:false,registrations,matches};
}
...
function generateCodes_(tournamentId,force){
  const t=tournament_(tournamentId);if(!t)throw new Error('Torneo no encontrado.');const sh=ss_().getSheetByName(TAB_CODIGOS);const existing=rows_(sh).filter(r=>String(r.torneo_id)===String(tournamentId));if(existing.length&&!force)return{ok:true,count:existing.length,message:'Los códigos ya existían.'};
  if(force&&existing.length){const vals=sh.getDataRange().getValues();const hm=headerMap_(sh);for(let i=vals.length;i>=2;i--)if(String(vals[i-1][hm.torneo_id-1])===String(tournamentId)&&upper_(vals[i-1][hm.estado-1])==='ACTIVO')sh.deleteRow(i)}
  const all=new Set(rows_(sh).map(r=>upper_(r.codigo)));const out=[];for(let i=0;i<Number(t.cupos||0);i++){let c;do{c=code_()}while(all.has(c));all.add(c);out.push([c,t.id,t.juego,'ACTIVO',1,0,new Date(),'',''])}
  if(out.length)sh.getRange(sh.getLastRow()+1,1,out.length,out[0].length).setValues(out);return{ok:true,count:out.length,message:`${out.length} códigos generados.`};
}
function activateTournament_(id){
  ...
  const hm=headerMap_(sh);sh.getRange(target._row,hm.estado).setValue('VIGENTE');sh.getRange(target._row,hm.inscripciones_estado).setValue('ABIERTAS');return{ok:true,message:'Torneo activado.'};
}
function setWinnerInstagram_(tournamentId,url){const ss=ss_(),tsh=ss.getSheetByName(TAB_TORNEOS),tr=tournamentRow_(tournamentId);if(!tr)throw new Error('Torneo no encontrado.');const hm=headerMap_(tsh);tsh.getRange(tr._row,hm.instagram_ganador).setValue(url||'');const wsh=ss.getSheetByName(TAB_WIN),wh=headerMap_(wsh),wr=rows_(wsh).find(r=>String(r.torneo_id)===String(tournamentId));if(wr)wsh.getRange(wr._row,wh.instagram_url).setValue(url||'');return{ok:true,message:'Instagram actualizado.'}}
...
function prepareFixture_(id){
  ...
  rebuildDerivedFixture_(id);return{ok:true,message:'Fixture preparado.'};
}
