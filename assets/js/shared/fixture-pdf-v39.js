...
function syncButtons(){cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{$$('[data-share-capture="fixture"]').forEach(btn=>{const panel=btn.closest('.gm-fixture-panel');if(!isKnockout(panel))return;btn.dataset.pdfReady='39';btn.textContent='DESCARGAR FIXTURE PDF ↓';btn.setAttribute('aria-label','Descargar fixture completo en PDF vectorial')})})}
...
function player(row){return{name:clean($('.gm-v36-player__name',row)?.textContent)||'Por definir',score:clean($('.gm-v36-score',row)?.textContent)||'-',penalty:clean($('.gm-v36-penalty',row)?.textContent),empty:row?.classList.contains('is-empty')||row?.classList.contains('gm-v36-player')&&clean($('.gm-v36-player__name',row)?.textContent).toLowerCase().includes('por definir')}}
function match(matchEl){const rows=$$('.gm-v36-player',matchEl);return{rows:[player(rows[0]),player(rows[1])]}}
...
async function renderPdf(bracket){
  ...
  const drawPlayer=(row,x,y,w,final=false)=>{const pad=4,rowH=38,scoreW=34,penW=row.penalty?34:0,g=5,nameW=w-pad*2-scoreW-penW-(row.penalty?g*2:g),fillName=row.empty?C.empty:(final?C.white:C.paper);p.rect(x+pad,y+4,nameW,rowH-8,fillName,null,1,6);p.text(row.name,x+pad+8,y+12,11,row.empty?C.muted:C.ink,nameW-14);const sx=x+pad+nameW+g;p.rect(sx,y+4,scoreW,rowH-8,final?C.white:C.paper,null,1,6);p.text(row.score||'-',sx,y+11,12,C.ink,scoreW,true);if(row.penalty){const px=sx+scoreW+g;p.rect(px,y+4,penW,rowH-8,C.pen,C.penBorder,1,6);p.text(row.penalty,px,y+11,12,C.white,penW,true)}};
  const drawMatch=(m,x,cy,w,final=false)=>{const top=cy-matchH/2;p.rect(x,top,w,matchH,final?C.empty:C.match,final?C.finalBorder:C.border,1,10);drawPlayer(m.rows[0],x,top,w,final);drawPlayer(m.rows[1],x,top+42,w,final);return{x,cy,w}};
  ...
  const ls=(pos.left[R-1]||[])[0],rs=(pos.right[R-1]||[])[0];if(ls)p.line(ls.x+ls.w,ls.cy,finalMatchX,finalCy);if(rs)p.line(rs.x,rs.cy,finalMatchX+finalMatchW,finalCy);
  const cupImg=$('.gm-v36-trophy img',bracket),cup=await imageToJpeg(cupImg);let cupBox=null;if(cup){const maxW=166,maxH=174,scale=Math.min(maxW/cup.width,maxH/cup.height),cw=cup.width*scale,ch=cup.height*scale,cx=centerX+(finalW-cw)/2,cy=Math.max(padTop+8,finalCardY-ch-18);cupBox={x:cx,y:cy,w:cw,h:ch}}else p.text('COPA',centerX,finalCardY-38,12,C.muted,finalW,true);
