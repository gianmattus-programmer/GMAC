(()=>{
  ...
  function trophyMarkup(t,small=false){const raw=t.trophyCover||t.trophy||'';const src=safeUrl(raw)||String(raw||'');if(src && !/^javascript:/i.test(src))return `<img decoding="async" src="${esc(src)}" alt="Copa de ${esc(t.title)}">`;return small?'<span>🏆</span>':'<div class="gm-trophy-placeholder">🏆</div><span>COPA DEL TORNEO</span><small>La imagen se añadirá cuando GMAC la asigne.</small>'}
